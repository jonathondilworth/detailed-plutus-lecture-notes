...
\\[ Given: Tx^{Alice} = \ 90 \ BTC\ \ and \  \sum_{i=0}^{X} Tx_{i}^{Bob} = 60 \ BTC \\]

\\[ Alice \ And \ Bob \ Pay \ Tom \ 110 \ BTC \ | \ Thus \ Change \ Recieved \ By \ Alice \ and \ Bob \\]

\\[ = \\]

\\[ \big(Alice \ | \ Tx^{Alice} - (110 \div 2) \ \big) = 35 \ BTC \\]

\\[ \big(Bob \ | { \ \Big[ \sum_{i=0}^{X} Tx_{i}^{Bob} \Big]\ - (110 \div 2) } \ \big) = 5 \ BTC\\]
